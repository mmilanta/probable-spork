...
Furthermore, because the match is balanced (one of the conditions we assumed), $\mgp_{0} = \h\left(\xgp_0\right) = 1/2$. Thus, $\mgp$ is a stochastic process which starts at $1/2$ and ends at $1$ or $0$, but even more interestingly, the expected value of $\mgp$ starts from $1/2$ and ends at the probability of winning the match.
...
$$
We now examine what happens if we perturb the match structure slightly from $1/2$. So we compute the probability of winning with $p = 1/2 + \varepsilon$.
\begin{align*}
...
Where $(1)$ is due to the fact that $\mghalf_0 = 1/2$, and translations do not contribute to the variance. In $(2)$ we use the observation that $\mghalf_i - \mghalf_{i-1} = \frac12 \Delta_G\left(\xghalf_{i-1}\right) b_{i-1}$. In $(3)$ we use the aforementioned independence to cancel out all the cross-terms. In $(4)$ we use the fact $\ex[b_i] = 0$, thus the variance is the expected value of the square.
...
\begin{align*}
    ...
    &\leq \underbrace{\ex\left[\sum_{i=0}^{\taughalf} \Delta_G\left(\xghalf_{i-1}\right)^2\right]}_{= 1 \text{ due to \eqref{eq:energy_identity}}} \ex\left[\sum_{i=0}^{\taughalf} 1\right]\\
    &= \ex\left[\sum_{i=0}^{\taughalf} 1\right] = \ex\left[\taughalf\right].
\end{align*}
Where $(1)$ is due to equation \eqref{eq:fairness_formula}. This concludes the proof.
\end{document}
