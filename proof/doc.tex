...
\begin{align*}
    \var \left(\mghalf_\taughalf\right) &= \var\left(\mghalf_0 + \sum_{i=1}^{\taughalf} \left(\mghalf_i - \mghalf_{i-1}\right)\right) \\
    &=\var\left(\sum_{i=1}^{\taughalf}\frac12 \Delta_G\left(\xghalf_{i-1}\right)  b_{i-1} \right)\\
    ...
    &\overset{(1)}{=}\frac14 \sum_{i=1}^{\taughalf} \var\left(\Delta_G\left(\xghalf_{i-1}\right) b_{i-1}\right)\\
    &\overset{(2)}{=}\frac14 \sum_{i=1}^{\taughalf} \ex\left[\Delta_G\left(\xghalf_{i-1}\right)^2\right]\\
\end{align*}

In $(1)$ we use the aformentioned indipendence to cancel out all the cross-terms. In $(2)$ we use the fact $\ex[b_i] = 0$, thus the variance is the expected value of the square.

Combining this with \eqref{eq:variance_target} we get
